...
namespace MiyakoCarryService.Assistant.Utils
{
    public static class Tools
    {
        ...
        public static int BindAndDispatch(LlmIntent intent)
        {
            ...
            if (Classification.CommandsNeedPosition.Contains(intent.CommandName) || Classification.CommandsNeedTargetId.Contains(intent.CommandName))
            {
                if (intent.OptionIndex.HasValue && Classification.OptionCommands.Contains(intent.CommandName))
                {
        ...
        private static Player[] SelectTargets(LlmIntent intent, Player[] aliveMembers)
        {
            ...
            if (intent.Selector == EIntentTargetSelector.All || (intent.Selector == EIntentTargetSelector.Unspecified && !intent.TargetIndex.HasValue && string.IsNullOrEmpty(intent.TargetCodeName)))
            {
                return aliveMembers;
            }
