...
{
    /// <summary>
    /// 试验层2：护航对敌优先级改造（参考 PitFireTeam 集火思路，以 MCS 实现风格落地）。
    /// 复制自 McsTestBrainLayer，优先级 202，仅战斗区选敌链路被改写（McsT2: 前缀 label）：
    ...
    {
        public McsTest2BrainLayer(BotOwner botOwner, int priority) : base(botOwner, priority)
        {
        ...
        public override Action GetNextAction()
        {
            try
            {
                ...
                if (fightActive)
                {
                    ...
                    if (goalEnemy == null)
                    {
                        return new Action(typeof(HoldPositionLogic), "McsT2:FightNoEnemy");
                    }
                        ...
                        && !McsBotPlayerData.HasAnyIntent(Intents.ShouldHoldPosition, Intents.ShouldFollowMe, Intents.ShouldKeepFormation))
                    {
                        if (TryGetCombatCoverTarget(mcsLeadPlayerPos, out var shiftCoverPos))
                        {
                            ...
                            if (_currentMoveTarget.HasValue)
                            {
                                _coverEnterTime = 0f;
                                ApplyMovePoint();
                                return new Action(typeof(GoToPointLogic), "McsT2:ShiftCover");
                            }
                        }
                        else
                        {
                    ...
                    {
                        return new Action(typeof(McsCoverPeakLogic), "McsT2:CoverPeak");
                    }
                    ...
                    {
                        return new Action(typeof(MeleeAttackLogic), "McsT2:MeleeAttack");
                    }
                    ...
                    {
                        return new Action(typeof(ShootToSmokeLogic), "McsT2:SmokeGrenad");
                    }
                    else
                    {
                        if (mcsLeadPlayerPos == null)
                        {
                            return new Action(typeof(HoldPositionLogic), "McsT2:Uninitialized");
                        }
                        ...
                        {
                            return new Action(typeof(McsDogFightLogic), "McsT2:DogFight");
                        }
                        ...
                        if (safeFire && haveBullets.Value)
                        {
                            if (goalEnemy.IsVisible)
                            {
                                ...
                                {
                                    return new Action(typeof(AttackMovingLogic), "McsT2:AttackMovingClose");
                                }
                                    ...
                                    && TryGetCombatCoverTarget(mcsLeadPlayerPos, out var coverPos))
                                {
                                    ...
                                    {
                                        ApplyMovePoint();
                                        return new Action(typeof(GoToPointLogic), heavySuppressed ? "McsT2:SuppressedSeekCover" : "McsT2:SeekCover");
                                    }
                                }
                                ...
                                {
                                    return new Action(typeof(AttackMovingLogic), "McsT2:AttackMoving");
                                }
                                else
                                {
                                    // 站桩射击 + 概率微走位（借鉴 SAIN StandAndShootAction.moveShoot）
                                    return new Action(typeof(McsStandAndShootLogic), "McsT2:StandAndShoot");
                                }
                            }
                            else
                            {
                                ...
                                {
                                    return new Action(typeof(RunToEnemyLogic), "McsT2:RushEnemyNear");
                                }
                                ...
                                {
                                    return new Action(typeof(McsSuppressFireLogic), "McsT2:SuppressFire");
                                }

                                // 保护冲脸：敌距队长 ≤50m（恢复旧层语义）或敌脆弱（换弹/治疗中）才冲
                                if (ShallRushEnemy(enemyToLeadSqrDist, goalEnemy, hasTravelTask))
                                {
                                    return new Action(typeof(RunToEnemyLogic), "McsT2:RushEnemy");
                                }
                                ...
                                if (McsBotPlayerData.HasIntent(Intents.ShouldGoToPoint))
                                {
                                    ...
                                    {
                                        ApplyMovePoint();
                                        return new Action(typeof(GoToPointLogic), "McsT2:GoToPointCommand");
                                    }
                                    else
                                    {
                                        return new Action(typeof(HoldPositionLogic), "McsT2:GoToLootTargetPosNotFound");
                                    }
                                }
                                ...
                                if (McsBotPlayerData.HasIntent(Intents.ShouldHoldPosition))
                                {
                                    ...
                                    {
                                        RefreshStuckTimer();
                                        return new Action(typeof(HealLogic), "McsT2:FightHealing4");
                                    }
                                    return new Action(typeof(HoldPositionLogic), "McsT2:HoldPositionCommand");
                                }
                                ...
                                if (needHeal && isEnemyPosLost)
                                {
                                    RefreshStuckTimer();
                                    ApplyMovePoint();
                                    return new Action(typeof(HealLogic), "McsT2:FightHealing5");
                                }
                                ...
                                if (sqrDistance >= TOO_FAR_FROM_LEAD_DISTANCE * 1 || tooClose)
                                {
                                    ...
                                    {
                                        ApplyMovePoint();
                                        return new Action(typeof(GoToPointLogic), tooClose ? "McsT2:TooClose" : "McsT2:TooFar");
                                    }
                                }
                                else
                                {
                                    if (_nextPatrolTime + 4f < time)
                                    {
                                        ...
                                        {
                                            ApplyMovePoint();
                                            return new Action(typeof(GoToPointLogic), "McsT2:Partoling");
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            RefreshStuckTimer();
                                            return new Action(typeof(HealLogic), "McsT2:FightHealing6");
                                        }
                                    }
                                }
                            }
                        ...
                        else
                        {
                            ...
                            if (!haveBullets.Value && TryGetCombatCoverTarget(mcsLeadPlayerPos, out var reloadCoverPos))
                            {
                                ...
                                {
                                    ApplyMovePoint();
                                    return new Action(typeof(GoToPointLogic), "McsT2:ReloadInCover");
                                }
                            }
        ...
        /// 对敌优先级扩展（McsTest2BrainLayer）：老板威胁敌（威胁窗口内攻击过老板）豁免滞回——
...
#if DEBUG
            if (!MiyakoCarryServicePlugin.EnableTest2BrainLayer.Value)
            {
                return false;
            }
#endif
