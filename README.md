...
| 设置项 | 说明 | 默认值 |
| --- | --- | --- |
| `启用语音` | 启用助手语音指令识别（STT + LLM）。开启后可通过语音指挥护航。 | `false` |
| `语音触发模式` | 触发方式：按键说话；自由发言 通过 VAD 自动录音无需按键。 | `PushToTalk` |
| `语音按键` | 触发 按键说话 模式的按键。自由发言 模式下忽略此项。 | `Empty` |
| `最长录音秒数` | 单次录音最长秒数，超过此长度将被截断。 | `15f` |
| `自由发言音量阈值` | 自由发言 模式下触发语音开始的 RMS 能量阈值。若轻声被忽略请调低。 | `0.01f` |
| `自由发言静音秒数` | 自由发言 模式下结束录音所需的静音时长。 | `1f` |
| `语音反馈字幕` | 每次语音指令下发后是否在游戏内显示通知。 | `true` |
| `录音设备` | 选择用于语音识别的录音设备；Default 表示系统默认设备。若列表为空请在系统设置中检查麦克风设备。 | `"Default"` |
| `语音转文字(STT) 服务商` | 云端语音转文字(STT)服务商 | `OpenAIWhisper` |
| `语音转文字(STT) API Key` | 所选语音转文字(STT)服务商的 API Key。 | `Empty` |
| `语音转文字(STT) Secret` | 双密钥服务商（讯飞/腾讯/百度/火山/阿里）的第二个密钥（Secret/Token），单密钥服务商留空。 | `Empty` |
| `语音转文字(STT) Base URL` | 可选自定义语音转文字(STT) Base URL（覆盖服务商默认值）。 | `Empty` |
| `语音转文字(STT) 模型` | 可选自定义语音转文字(STT) 模型名（服务商相关），留空使用默认。 | `Empty` |
| `语音转文字(STT) 语种` | 语音转文字的语种提示（BCP-47，如 zh-CN、en-US、ja-JP）。 | `Empty` |
| `语音转文字(STT) 超时秒数` | 每次语音转文字请求的超时秒数。 | `15` |
| `大语言模型(LLM) 服务商` | 云端大语言模型(LLM)服务商。OpenAI兼容 覆盖 OpenAI / DeepSeek / Moonshot / Ollama / vLLM 等 | `OpenAICompatible` |
| `大语言模型(LLM) API Key` | 所选大语言模型(LLM)服务商的 API Key。 | `Empty` |
| `大语言模型(LLM) Secret` | 双密钥服务商的第二密钥（Secret/Token）；单密钥服务商可留空。 | `Empty` |
| `大语言模型(LLM) Base URL` | 可选自定义大语言模型(LLM) Base URL（仅对 OpenAI兼容 服务商生效，覆盖服务商默认值）。 | `Empty` |
| `大语言模型(LLM) 模型` | 大语言模型(LLM) 模型名。服务商相关；OpenAI兼容 默认 deepseek-v4-flash。 | `"deepseek-v4-flash"` |
| `大语言模型(LLM) 系统提示词` | 可选自定义系统提示词，将拼接在语音指令模板前。 | `Empty` |
| `大语言模型(LLM) 采样温度` | 大语言模型(LLM) 采样温度，范围 0到2。值越低结果越确定。 | `0.2` |
| `大语言模型(LLM) 最大 Tokens` | 大语言模型(LLM) 单次最大 Tokens 输出，影响成本与延迟。 | `10107` |
| `大语言模型(LLM) 超时秒数` | 每次大语言模型(LLM)请求的超时秒数。 | `15` |
| `大语言模型(LLM) 思考强度` | LLM 思考强度（reasoning effort）：default / low / medium / high / max。仅提供选项，此模型是否支持需根据实际情况而定；default 表示不传该参数，不支持的模型通常忽略该参数。 | `"low"` |
| `HTTP 代理地址` | LLM/STT 请求使用的 HTTP 代理主机名或 IP（如 127.0.0.1）；留空表示直连。配置后所有请求（含本地地址）均经代理转发。 | `Empty` |
| `HTTP 代理端口` | HTTP 代理端口（如 7890）；需与代理地址同时配置，留空表示直连。 | `Empty` |

### Z. 调试

| 设置项 | 说明 | 默认值 |
| --- | --- | --- |
| `语音转文字(STT) 调试开关` | 开启后使用当前配置进行录音并转写，转写文本会覆盖显示在 语音转文字(STT) 调试文本中。 | `false` |
| `语音转文字(STT) 调试文本` | 最近一次录音的转写结果。 | `Empty` |
| `自由发言监听状态` | 实时显示自由发言的语音检测状态（rms 能量 / 是否在说话 / 静音累计时长） | `Empty` |
| `大语言模型(LLM) 发送测试` | 发送 STT 调试文本到 大语言模型(LLM) 进行测试，回复或报错信息会覆盖显示在 大语言模型(LLM) 返回结果中。 | `false` |
| `大语言模型(LLM) 返回结果` | 最近一次 大语言模型(LLM) 测试的回复或错误信息。 | `Empty` |
| `调试识别指令` | 开启后，在 STT 调试模式下转写完成后自动调用 大语言模型(LLM) 进行指令识别测试，识别结果显示在『识别指令结果』中。 | `false` |
| `识别指令结果` | 最近一次自动指令识别的结果，显示 大语言模型(LLM) 实际将会调用的指令情况。 | `Empty` |
| `播放录音` | 播放最近一次语音录制的内容（DEBUG 回放，需先有一次成功的录音） | `false` |
