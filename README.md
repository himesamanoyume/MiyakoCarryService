...
| OrderPendingPaymentTime | 通过 宫子商人 下单 `护航` 或发放补票时，行动任务的有效等待时间，单位为秒。 | `900` 秒（15 分钟） |
...
| 设置项 | 说明 |
| --- | --- |
| 绘制护航调试信息 | 是否在战局中绘制 `护航` 的调试信息。 |
| 护航调试信息字体大小 | 绘制 `护航` 调试信息时的字体大小。 |
