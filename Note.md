...
- 助手插件需要进一步适配，支持语音控制调节配置项状态，如是否进行掠夺(切换开关状态)，语音调整战利品价值阈值等所有已注册的Mcs配置项的值
- - 增加TTS(文字转语音)适配
- 护航强度是否需要支持自己调整
...
- 当护航全身完全没有子弹、或缺少弹匣时，应主动掠夺范围内的符合或兼容当前武器的对应子弹、弹匣，即便掠夺战利品开关未打开
- 当护航全身完全没有能够治疗骨折、大出血、小出血、手术的物品时，应主动掠夺范围内的符合条件的对应医疗物品（比如如果有一个物品能够同时治疗大出血和小出血，就视为无需再找这两个效果的对应医疗品，但是手术包在治疗时能移除骨折应该例外，不视作其能够治疗骨折，此时若没有骨折环等物品时视为应该去寻找），即便掠夺战利品开关未打开
- AttackMoving需要根据敌人是否可见选择当前的径直前往敌人位置方式，若敌人不可见则仍然径直前往敌人位置，若敌人可见则使用EFT原生或SAIN的斜角度移动
- ~~考虑到EFT存在持续激活敌人的bug，需要考虑用单独一个Layer去进行所有判断~~
- - 统一Layer后，对于间隔进行计算的目标位置计算，应该独立出来，因为当前的`_currentMoveTarget`会因为受到Action和Layer是否结束的影响，不一定能够及时按照设定的时间间隔进行重算更新，导致很多时候护航的目标点常常得不到及时更新
- - ~~统一Layer开关作为新的Y.实验性分类使用~~
