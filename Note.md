...
- ~~我现在有一个极其重要的新需求：实现护航在过期时不再立刻删除好友和删除护航存档，也不立刻删除对应的订单信息，而是保留着好友和订单的同时，让其受到邀请时直接拒绝，并且是以一个订单内的护航为范围的暂时保留，右键菜单中增加一个结单选项，对于订单到期的护航，除非玩家手动对护航右键进行结单，才会真正进行删除和删档，若未到期则也无法结单。同时相对的再增加一个续订选项，服务端需要生成新的订单任务并对订单内的所有人都进行续订，续订的时间与最初设定的时间相同，续订完成后，过期时间也相应地延后，并且订单没过期的期间也可以进行续订。如何实现，进行确认并给出计划~~
- - 当前会出现服务端订单立即删除护航存档的情况
