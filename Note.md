...
- **测试游戏内实际表现**
- OpenAI Whisper是不是也可以改成文本为：OpenAI兼容？
- LLM发送测试原本明明只是要求返回简单结果，结果现在却进行了指令识别。要求返回成功时依旧为pong
- 对于通常必填的几个配置项，比如BaseUrl，显示的Key名后加个"*"，模仿必填的样式。对于选填的配置项，则在Description中末尾加上"(选填)"
