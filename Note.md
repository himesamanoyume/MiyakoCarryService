...
- 是否需要将助手与宫子商人的LLM调用指令相关的工具重命名为MCP并集中（如McpService，最后注入并引用最多的类）
- ~~宫子商人支持LLM续订、结算（帮我续订/结算n7fd这个护航的那支订单）~~
- 修复LLM/STT服务商选择失效的问题
- ~~对于一些读音重复的指令，可以LLM自己思考是否是STT转写的偏差，比如驻守被转写成助手，这个时候LLM应该自己将转写内容往驻守指令上靠~~
- APIKey需要改成密文类型的密码输入框，最好是CustomDrawer，用"*"隐藏前三位之后的字符
- ~~调试中再加入一个播放已录音文件的按钮~~

1. 我说的服务商选择失效，是指服务商的下拉选择框没有正常生效，我无法进行选择了。当然如果你确实修复了切换服务商时没有实时生效的问题，那就保留着。
2. APIKey密文框不要太长，需要与原本的BepInEx配置管理器的输入框长度一直，并且也要保留Reset按钮
3. 既然已可以直接播放录音了，那么可以将保存录音文件的代码移除了
