...
- ~~测试转移是否还会让死亡的护航再次生成~~
- - ~~修复后不再会生成了，但是有了新的问题：在转移后的战局中，对于_mcsSquadDict中并没有健壮的判断逻辑，因为护航死亡后，在转移后的战局中重新构建_mcsSquadDict时，并不会过滤掉死去的护航的id，而又因为其不会生成，所以_mcsSquadDict中会存在player为空引用的情况~~
- **问题：目前第二次撤离似乎还是会导致撤离决策没有执行**
- 开始进行无Fika、Fika联机、SAIN、APBS、ORBIT测试（要求是至少两次进入战局，至少撤离一次）
...
- 对护航新增结算、续订两个选项，并调整订单结算机制：现在服务过期时不再会立即让护航删除好友，而是会进入到过期状态，你可以选择进行结算或是续订。结算成功才会真正删除护航的所有数据，而续订则会再次派发相同时长相同费用的新订单，续订任务完成后即可延长服务过期时间
- 新增指令：清扫区域
- 护航不再会拆除玩家布设的绊雷
