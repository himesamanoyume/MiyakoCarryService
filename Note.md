...
```
- **实现护航获取掠夺目标**（掠夺第一阶段）
- - 1. ~~让护航获取当前范围内的第一优先级战利品（即便是敌人身上的物品也算在内）~~**(未验证)**
- - - ~~完成了当前无掠夺目标战利品时获取新目标的部分（未经测试）、还剩下当前掠夺目标战利品因某种原因而解除的部分（比如掠夺完成、或超出距离等）~~
- - - - ~~补充：还需要先判断其是否可拿~~
- - 2. ~~让护航前往第一优先级战利品的位置~~**(未验证)**
- - - ~~应该记录目标战利品及其RootItem的信息来锁定，因为非最外部的RootItem其一定没有Transform~~
- - 3. **护航执行掠夺**
- - - 掠夺补充：先通过目标战利品确定根物品的位置来前往，而到了位置后**如果是容器，则应该一并将其中的物品也全都搜刮，而不是仅仅拿走最高优先级战利品**
- 实现护航根据已掠夺的战利品做出后续行为（掠夺第二阶段）
- - 1. 根据自身是否缺医疗物品、手术包去进行掠夺对应物品
- - 1. 根据自身是否缺水缺能量、去进行掠夺对应物品
- - 3. 根据自身已掠夺的战利品价值，判断是否应该上贡
- 在进行在掠夺战利品的行动中，`原则就是老板用得上的，就优先留给老板，用不上的就自己拿来用`：
- - 如果是武器，则先判断是否大于价值阈值，再判断老板的武器总价值是否大于该武器的总价值，若大于则再判断武器位是否有空位，若无空位再放入背包，标记为贡品。如果小于老板的武器价值，又大于当前武器的价值，无视价值阈值，都应该放入空武器位，无空位则替换最低价值武器的栏位
- - 如果是装备，只判断老板对应部位的护甲/插板的防护等级是否低于此装备，如果是则放入背包，标记为贡品。如果小于老板的武器价值，又大于当前武器的价值，无视价值阈值，都应该放入空武器位，无空位则替换最低价值武器的栏位
- - 如果是子弹，只判断穿透、肉伤是否大于设置阈值，若大于则都拿，不看价值
- - 其他物品则需要优先判断价值
- - 战利品需要存在忽视属性，否则可能导致重复被检查
- - - ~~搜索过的战利品打上忽略属性，当其TraderController被触发过之后意味着其物品有变动，此时取消忽略属性。但是在联机状态下，不同老板的需求不同，所以不能简单的设置一个bool值来标记忽略，而是标记McsAIBossPlayer列表，当列表内有对应的老板时，说明这个物品对该老板来说应该被忽视，而其他老板则不一定~~(已无法满足需求)
